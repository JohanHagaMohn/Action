window.addEventListener("DOMContentLoaded", () => {
  ...
  window.addEventListener("scroll", function scrolling() {
    if (window.scrollY > window.innerHeight * 0.15) {
      document.querySelector("#down").style.transform = "opacity 2s ease-out";
  ...
  function swapID(items, prefix) {
    ...
    for (let i = 1; i < carouselItems + items; i++) {
      document.querySelector(`#${prefix}to${i}`).id = `${prefix}circle${i}`;
    }
  }
  ...
  function scrollNodes() {
    ...
    setTimeout(() => {
      ...
      swapID(3, "m");
      setTimeout(() => {
        if (document.querySelector("#circle1").style.borderColor == green) {
          scrollNodes();
        }
      }, 350);
    }, 350);
  ...
  function changeColor(id) {
    ...
    if (document.querySelector(`#circle${id}`).style.borderColor == green) {
      if (first) {
        document.querySelector(`#mcircle${id}`).style.borderColor = green;
        scrollNodes();
      } else {
        document.querySelector(`#mcircle${id}`).style.borderColor = changeColor(
          id - 1
        )
          ? green
          : red;
    ...
    element.addEventListener("click", () => {
      ...
      for (let i = 1; i < carouselItems + 1; i++) {
        if (element.id == `circle${i}`) {
          ...
          if (
            i < carouselItems &&
            document.querySelector(`#circle${i + 1}`).style.borderColor == green
          ) {
            document.querySelector(`#mcircle${i + 1}`).style.borderColor =
              document.querySelector(`#circle${i}`).style.borderColor == red
                ? red
                : green;
          }
